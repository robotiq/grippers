...
//! One gPO count of opening on a 2F-85 (0.37 mm), the step the filter has to
//! survive, and the slowest speed it has to stay clear of.
...
//! Feed positions advancing at \p velocity from zero, \p period apart, for
//! \p samples periods.
double ramp(VelocityEstimator& estimator, double velocity, std::chrono::nanoseconds period, int samples)
{
   ...
   for(int sample = 0; sample <= samples; ++sample)
   {
      ...
      result = estimator.update(velocity * seconds(now), now);
   }
   return result;
}
} // namespace
...
{
   VelocityEstimator estimator(kTimeConstant);
   // The slowest a 2F-85 moves, sampled at 100 Hz for five time constants.
...
TEST(TestVelocityEstimator, standing_still_decays_to_nothing)
{
   ...
   // The fingers stop: from here the same position repeats.
   const double stopped = 0.050 * seconds(kMoved);
   double velocity = 0.0;
...
TEST(TestVelocityEstimator, one_count_at_rest_reads_as_that_count_over_the_time_constant)
{
   ...
   // A stationary gripper whose reading flips one count. The raw difference
   // would be 37 mm/s at this sampling rate, twice the gripper's top speed.
   const double bump = estimator.update(0.040 + kCount, 200ms);
   EXPECT_GT(bump, 0.0);
   EXPECT_LT(bump, kCount / seconds(kTimeConstant));
   EXPECT_LT(bump, kSlowest);
}

TEST(TestVelocityEstimator, a_quantized_ramp_reads_as_the_rate_it_averages)
{
   // What the driver sees: a byte-quantized position sampled five times per
   // exchange, so four samples in five repeat and the fifth jumps a count.
   VelocityEstimator estimator(kTimeConstant);
   ...
   for(int sample = 0; sample <= 800; ++sample)
   {
      ...
      const double exact = kRate * seconds(now);
...
TEST(TestVelocityEstimator, two_samples_the_clock_cannot_separate_carry_no_rate)
{
   ...
   // Neither refused sample became the reference: this one is still a
   // millimetre on from 0.001, so it reads as more motion, not less.
...
TEST(TestVelocityEstimator, reset_forgets_the_gap_the_gripper_was_away_for)
{
   ...
   estimator.reset();
   EXPECT_DOUBLE_EQ(estimator.velocity(), 0.0);
   // The far side of the gap is a first sample again, not a stroke of travel.
